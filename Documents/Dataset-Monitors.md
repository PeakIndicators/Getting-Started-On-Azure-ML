...
|Frequency|	The frequency that will be used to schedule the pipeline job and analyze historical data if running a backfill.|Options include day, week or month.	Each run compares data in the target dataset according to the frequency: * Day: Compare most recent complete day in target dataset with baseline; * Week: Compare most recent complete week (Monday - Sunday) in target dataset with baseline; * Month: Compare most recent complete month in target dataset with baseline
